"""Phase 1 Group N バックテスト（施策A: ボラティリティレジーム連動 動的シグナル閾値）

I1 設定（CAGR 7.72%、Max DD 24.96%、PF 1.297、Sharpe 0.382）の Sharpe 改善を目的として、
TOPIX 実現ボラティリティが低い局面（低ボラレジーム）での BUY 閾値引き上げ効果を検証する。
...
Group N シナリオ:
  N0_i1_ref      : no vol regime (pure I1 reference)
  N1a_vol15_hi62 : adaptive_threshold_vol_regime=True, topix_vol_window=20, topix_vol_low_threshold=0.15, adaptive_threshold_hi=0.62
  N1b_vol12_hi62 : adaptive_threshold_vol_regime=True, topix_vol_window=20, topix_vol_low_threshold=0.12, adaptive_threshold_hi=0.62
  N1c_vol15_hi60 : adaptive_threshold_vol_regime=True, topix_vol_window=20, topix_vol_low_threshold=0.15, adaptive_threshold_hi=0.60

採択基準:
  Sharpe > 0.5                   : 採用
  0.382 < Sharpe <= 0.5          : 参考値として記録、I1 継続採用
  全シナリオ Sharpe <= 0.382     : 施策A 効果なし → 施策B へ進む

Usage:
    python backtest/backtest_improvement_plan/run_phase1_group_n.py
...
ARTIFACTS_ROOT = REPO_ROOT / "artifacts" / "backtest" / "backtest_phase1_group_n"
...
_GROUP_N: list[dict] = [
    {
        "name": "N0_i1_ref",
        "group": "N",
        "adaptive_threshold_vol_regime": False,
        "topix_vol_window": 20,
        "topix_vol_low_threshold": 0.15,
    ...
    },
    {
        ...
        "adaptive_threshold_hi": 0.62,
        "desc": "低ボラ(vol<12%)で閾値0.62に引き上げ（より厳しいトリガー）",
    },
    {
        "name": "N1c_vol15_hi60",
        "group": "N",
        ...
        "topix_vol_window": 20,
        "topix_vol_low_threshold": 0.15,
        "adaptive_threshold_hi": 0.60,
        "desc": "低ボラ(vol<15%)で閾値0.60に引き上げ（より緩い閾値）",
    },
]

ALL_SCENARIOS: list[dict] = _GROUP_N
...
def _build_command(db_path: Path, scenario: dict, output_dir: Path) -> list[str]:
    ...
    if scenario.get("adaptive_threshold_vol_regime"):
        cmd.append("--adaptive-threshold-vol-regime")
        cmd += ["--topix-vol-window", str(scenario["topix_vol_window"])]
        cmd += ["--topix-vol-low-threshold", str(scenario["topix_vol_low_threshold"])]
        cmd += ["--adaptive-threshold-hi", str(scenario["adaptive_threshold_hi"])]
    return cmd
...
def _run_batch(args: tuple) -> list[dict]:
    ...
    for scenario in scenario_batch:
        ...
        record = {
            ...
            "adaptive_vol_regime": scenario.get("adaptive_threshold_vol_regime", False),
            "topix_vol_window": scenario.get("topix_vol_window", ""),
        ...
        print(
            f"[DONE] {name:<22}"
            f"  vol_regime={str(scenario.get('adaptive_threshold_vol_regime', False)):<6}"
            f"  vol_thr={scenario.get('topix_vol_low_threshold', '')}"
            f"  hi_thr={scenario.get('adaptive_threshold_hi', '')}"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Phase 1 Group N バックテスト（施策A: ボラティリティレジーム連動 動的シグナル閾値）"
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    if success:
        print("\n--- Group N: ボラティリティレジーム連動 動的シグナル閾値 検証結果 ---")
        print(
            f"{'name':<22} {'vol_regime':<11} {'vol_window':>10} {'vol_thr':>8} {'hi_thr':>7}"
        ...
        for r in success:
            ...
            print(
                f"{r['name']:<22}"
                f" {str(r.get('adaptive_vol_regime', '')):^11}"
                f" {str(r.get('topix_vol_window', '')):>10}"
                f" {str(r.get('topix_vol_low_threshold', '')):>8}"
                f" {str(r.get('adaptive_threshold_hi', '')):>7}"
        ...
        if adopted_scenarios:
            ...
            improved = [
                r for r in success if r["name"] != "N0_i1_ref" and _get_sharpe(r) > _I1_SHARPE
            ]
            if improved:
                ...
                print(
                    ...
                    "施策A（ボラティリティレジーム連動閾値）効果なし → 施策B へ進む"
                )
