"""Phase 2 Group X — quality_score 閾値の厳格化検証 (Issue #391)

Issue #388 分析より、quality_score_min=-0.30 フィルターが 2021 年に
+39 件の過剰エントリーを招き、その多くがストップロスで損失した。
閾値を厳格化（0.0・0.30）して 2021〜2023 型の過剰エントリーを抑制できるか検証する。

ベース: V1_score（score_drop_atr_gate=1.0 + quality_score_min=-0.30）
シナリオ:
  X0_ref   : V1 + quality_score_min=-0.30（現採択設定）
  X1_000   : V1 + quality_score_min=0.0（中間）
  X2_030   : V1 + quality_score_min=0.30（厳格）
  X3_none  : V1 + quality_score_min なし（U0_ref 相当）
...
Usage:
    python backtest/backtest_improvement_plan/run_phase2_group_x.py
...
ARTIFACTS_ROOT = REPO_ROOT / "artifacts" / "backtest" / "backtest_phase2_group_x"
...
# quality_score_min = None はフィルターなし（X3_none）
_SCENARIOS: list[dict] = [
    {
        "name": "X0_ref",
        "group": "X",
        "quality_score_min": -0.30,
        "desc": "V1 + quality_score_min=-0.30（現採択設定）",
        "is_reference": True,
    },
    {
        "name": "X1_000",
        ...
        "desc": "V1 + quality_score_min=0.0（中間）",
        "is_reference": False,
    },
    {
        "name": "X2_030",
        "group": "X",
        "quality_score_min": 0.30,
        "desc": "V1 + quality_score_min=0.30（厳格）",
        "is_reference": False,
    },
    {
        "name": "X3_none",
        ...
        "desc": "V1 + quality_score_min なし（U0_ref 相当）",
        "is_reference": False,
    },
]
...
def _build_command(db_path: Path, scenario: dict, output_dir: Path) -> list[str]:
    ...
    qmin = scenario.get("quality_score_min")
    if qmin is not None:
        cmd += ["--quality-score-min", str(qmin)]
    return cmd
...
def main() -> None:
    ...
    print(f"[INFO] Group X バックテスト開始  workers={workers}  出力={output_dir}")
    print(
    ...
    print(f"{'=' * 60}")
    header = (
        f"{'シナリオ':<12}  {'quality_min':>12}  "
        f"{'CAGR':>8}  {'Sharpe':>7}  {'MaxDD':>7}  {'PF':>6}  {'Trades':>7}"
    )
    print(header)
    print("-" * 65)
    for r in all_results:
        if r.get("error"):
            ...
            continue
        qmin = r.get("quality_score_min")
        qmin_str = str(qmin) if qmin is not None else "None"
        mark = " *" if _meets_all(r) else ("  ref" if r.get("is_reference") else "")
        print(
            f"  {r['name']:<12}  {qmin_str:>12}"
            f"  {_pct(r.get('cagr')):>8}"
