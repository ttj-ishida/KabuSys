"""Phase 2 Group W — V3 エントリーフィルター gate 緩和検証 (Issue #390)

Issue #388 分析より、V3（entry_3d_max_abs_return=±5%）は過剰フィルタリングで
CAGR 0.2% まで崩壊した。0-7日ストップロス損失を構造的に削減するため、
gate を ±8%・±10%・±12% に緩和して V1（score_drop_atr_gate=1.0）との
複合効果を検証する。

ベース: V1_score（score_drop_atr_gate=1.0 + U1_t4設定）
シナリオ:
  W0_ref   : V1 のみ（Group V 採択設定、フィルターなし）
  W1_08    : V1 + V3（gate=±8%）
  W2_10    : V1 + V3（gate=±10%）
  W3_12    : V1 + V3（gate=±12%）
  W4_08_v3 : V3 のみ（gate=±8%）  ← V1 なし単体効果
  W5_10_v3 : V3 のみ（gate=±10%） ← V1 なし単体効果
...
Usage:
    python backtest/backtest_improvement_plan/run_phase2_group_w.py
...
ARTIFACTS_ROOT = REPO_ROOT / "artifacts" / "backtest" / "backtest_phase2_group_w"
...
def main() -> None:
    ...
    print(f"[INFO] Group W バックテスト開始  workers={workers}  出力={output_dir}")
    print(
    ...
    print(f"{'=' * 60}")
    header = f"{'シナリオ':<14}  {'CAGR':>8}  {'Sharpe':>7}  {'MaxDD':>7}  {'PF':>6}  {'Trades':>7}"
    print(header)
    print("-" * 60)
