"""Phase 2 Group S — マルチレジーム適応閾値（施策A 3 値レジーム拡張）検証 (Issue #376)

2 値レジーム（低ボラのみ閾値引き上げ）を 3 値（低ボラ／中ボラ／高ボラ）に拡張し、
高ボラ局面での積極エントリー効果を検証する。

ベース設定: Group R 採択設定（R3: max_positions=7, max_utilization=40%）

IS 参照値（R3、Phase2_Backtest_Strategy.md Section 48）:
  CAGR 9.01%、Sharpe 0.445、MaxDD 26.87%、PF 1.402

シナリオ:
  S0_p2_ref : 現行 2 値レジーム参照（topix_vol_high_threshold なし）
  S1        : vol_high=0.25, thr_lo=0.55（高ボラ積極化）
  S2        : vol_low=0.10, vol_high=0.20, thr_hi=0.63, thr_lo=0.55（閾値帯拡大）
  S3        : vol_high=0.25, thr_lo=0.58（高ボラ緩和のみ・低ボラ維持）
  S4        : vol_high=0.25, thr_hi=0.65, thr_lo=0.55（低ボラ厳格化 + 高ボラ積極化）
  S5        : vol_low=0.15, vol_high=0.30, thr_lo=0.55（vol 帯を広め設定）
...
Usage:
    python backtest/backtest_improvement_plan/run_phase2_group_s.py
...
ARTIFACTS_ROOT = REPO_ROOT / "artifacts" / "backtest" / "backtest_phase2_group_s"

# ---------------------------------------------------------------------------
# R3 ベース固定パラメータ（施策A パラメータ以外）
# ---------------------------------------------------------------------------
...
def _run_batch(args: tuple) -> list[dict]:
    ...
    for scenario in scenario_batch:
        ...
        print(
            f"[DONE] {name:<12}"
            f"  vol_lo={str(scenario.get('topix_vol_low_threshold')):>5}"
            f"  vol_hi={'—' if scenario.get('topix_vol_high_threshold') is None else str(scenario.get('topix_vol_high_threshold')):>5}"
            f"  thr_hi={str(scenario.get('adaptive_threshold_hi')):>5}"
            f"  thr_lo={'—' if scenario.get('adaptive_threshold_lo') is None else str(scenario.get('adaptive_threshold_lo')):>5}"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Phase 2 Group S — マルチレジーム適応閾値（施策A 3 値レジーム）検証"
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    print("\n" + "=" * 130)
    print("【Group S 結果】R3 固定 / 施策A パラメータ変動（2017-2025）")
    print(
        f"  IS 参照（S0）: CAGR {_IS_CAGR:.2%}  Sharpe {_IS_SHARPE:.3f}"
        f"  MaxDD {_IS_MAX_DD:.2%}  PF {_IS_PF:.3f}"
    )
    print("=" * 130)
    print(
        f"  {'シナリオ':<12}  {'vol_lo':>6}  {'vol_hi':>6}  {'thr_hi':>6}  {'thr_lo':>6}"
    ...
    for r in success:
        ...
        better = (r.get("sharpe") or 0) > ref_sharpe and r["name"] != "S0_p2_ref"
        marker = " [ALL]" if all_ok else (" [>S0]" if better else "")
        print(
            f"  {r['name']:<12}"
            f"  {str(r.get('topix_vol_low_threshold') or '—'):>6}"
            f"  {'—' if r.get('topix_vol_high_threshold') is None else str(r.get('topix_vol_high_threshold')):>6}"
            f"  {str(r.get('adaptive_threshold_hi') or '—'):>6}"
    ...
    improved = [
        r
        for r in success
        if (r.get("sharpe") or 0) > (ref["sharpe"] if ref else 0) and r["name"] != "S0_p2_ref"
    ]
    ...
    if adopted:
        ...
        print(
            ...
            f"（Phase2_Backtest_Strategy.md Section 49 に記録）"
        )
        decision = {
            "verdict": "ADOPTED",
        ...
        print(f"  Sharpe が S0 を上回るシナリオ: {[r['name'] for r in improved]}")
        print(
            f"  → {best['name']} を Phase 2 ベースとして採用"
            f"（Sharpe 0.5 未達は Group T との組み合わせで改善を狙う）"
        )
        decision = {
            "verdict": "IMPROVED",
            "best_scenario": best["name"],
        ...
        print("  全シナリオで S0（現行 2 値レジーム）以下")
        print("  → マルチレジーム拡張では構造改善不可。別アプローチを検討")
        decision = {"verdict": "NO_IMPROVEMENT", "reason": "All scenarios <= S0"}
