...
def main():
    ...
    print(SEP)
    print(f"{'Year':>4}  {'U0_ref':>12}  {'U1_t4':>12}  {'V0_ref':>12}  {'V1_score':>12}  {'U0-U1 diff':>12}")
    print("-" * 72)
    for y in COMPARE_YEARS:
        ...
        print(f"{y:>4}  {u0:>+12,.0f}  {u1:>+12,.0f}  {v0:>+12,.0f}  {v1:>+12,.0f}  {u0-u1:>+12,.0f}{mark}")

    # ── 2. Monthly PnL: U0_ref vs U1_t4 (2021-2023) ─────────────────────────
    print(f"\n{SEP}")
    print("[ 2 ] Monthly Realized PnL -- U0_ref vs U1_t4 (2021-2023, JPY)")
    print(SEP)
    print(f"  {'Mth':>3}  {'U0/2021':>10}  {'U1/2021':>10}  {'diff':>9}  |  {'U0/2022':>10}  {'U1/2022':>10}  {'diff':>9}  |  {'U0/2023':>10}  {'U1/2023':>10}  {'diff':>9}")
    print(f"  {'-'*100}")
    for m in range(1, 13):
        row = f"  {m:>3}"
    ...
    for ym in worst[:12]:
        ...
        print(f"  {ym[0]}/{ym[1]:02d}  {u1v:>+12,.0f}  {u0v:>+12,.0f}  {v1v:>+12,.0f}  {u0v-u1v:>+12,.0f}{mark}")
    ...
    print(SEP)
    print(f"  {'Year':>4}  {'U0 trades':>10}  {'U1 trades':>10}  {'diff':>8}  {'U0 PnL':>12}  {'U1 PnL':>12}  {'diff':>12}")
    print(f"  {'-'*75}")
    u0_by_yr: dict[int, list] = defaultdict(list)
    u1_by_yr: dict[int, list] = defaultdict(list)
    for t in closed["U0_ref"]: u0_by_yr[t["year"]].append(t)
    for t in closed["U1_t4"]:  u1_by_yr[t["year"]].append(t)
    for y in COMPARE_YEARS:
        u0n   = len(u0_by_yr[y]); u1n = len(u1_by_yr[y])
        u0pnl = sum(t["pnl"] for t in u0_by_yr[y])
        u1pnl = sum(t["pnl"] for t in u1_by_yr[y])
        mark = "  <=" if y in TARGET_YEARS else ""
        print(f"  {y:>4}  {u0n:>10}  {u1n:>10}  {u1n-u0n:>+8}  {u0pnl:>+12,.0f}  {u1pnl:>+12,.0f}  {u0pnl-u1pnl:>+12,.0f}{mark}")

    # ── 8. V1_score vs V0_ref monthly diff in 2022-2023 ────────────────────
    print(f"\n{SEP}")
    print("[ 8 ] V1_score vs V0_ref monthly diff (2022-2023, reason for extra loss)")
    print(SEP)
    print(f"  {'YM':>7}  {'V0_ref':>12}  {'V1_score':>12}  {'diff V0-V1':>12}  {'hold_d diff (V1-V0 avg)':>24}")
    print(f"  {'-'*70}")
    ...
    for y in [2022, 2023]:
        for m in range(1, 13):
            ...
            if abs(diff) > 10000:
                ...
                print(f"  {y}/{m:02d}  {v0v:>+12,.0f}  {v1v:>+12,.0f}  {diff:>+12,.0f}  V0_avg={ah0:.1f}d  V1_avg={ah1:.1f}d")
