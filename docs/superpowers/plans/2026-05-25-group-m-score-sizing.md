# Group M: シグナル強度ベースサイジング 実装計画

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `run_phase1_group_m.py` を作成し、allocation_method（equal/score）と max_positions（2/3）の組み合わせ4シナリオでバックテストを実行できるようにする。

**Architecture:** `run_phase1_group_k.py` と同一の骨格（ProcessPoolExecutor + _build_command + CSV出力）を流用する。engine.py・run.py への変更は不要で、既存の `--allocation-method` / `--max-positions` CLI フラグを利用する。

**Tech Stack:** Python 3.10+, subprocess, ProcessPoolExecutor, csv, json, pathlib

---

### Task 1: run_phase1_group_m.py の作成

**Files:**
- Create: `backtest/backtest_improvement_plan/run_phase1_group_m.py`

**設計の要点（Group K との差分）:**
- `_COM` に `allocation_method` / `max_positions` を含めない（シナリオごとに異なる）
- 各シナリオ dict に `allocation_method: str` と `max_positions: int` を持たせる
- `_build_command()` でシナリオの値を CLI に渡す
- `_format_scenario()` は `allocation_method` と `max_positions` を表示する
- CSV フィールドは `allocation_method`, `max_positions` を持つ
- 採択ロジック: Sharpe > 0.5 → 採用、Sharpe > 0.382 → 参考記録、それ以外 → Group N へ

**シナリオ定義:**

| name | allocation_method | max_positions | desc |
|---|---|---|---|
...
**共通パラメータ（I1 固定）:**
```
cash=1_000_000, max_position_pct=0.22, risk_pct=0.005,
max_utilization=0.30, use_ma200_filter=True,
stop_loss_pct=0.09, max_holding_days=60, trailing_stop_atr=2.0,
threshold=0.58, dd_stop=0.12, dd_timeout=30,
weak_bear=1.00, strong_bear=1.00,
start="2017-01-01", end="2025-12-31"
```

- [ ] **Step 1: ファイルを作成する**

`backtest/backtest_improvement_plan/run_phase1_group_k.py` を参考に、以下の内容で `backtest/backtest_improvement_plan/run_phase1_group_m.py` を作成する。
...
def main() -> None:
    ...
    if success:
        ...
        print("\n採択判断:")
        m0 = next((r for r in success if r["name"] == "M0_i1_ref"), None)
...
- [ ] **Step 2: ruff でフォーマット・lint チェック**

```powershell
python -m ruff check backtest/backtest_improvement_plan/run_phase1_group_m.py
python -m ruff format backtest/backtest_improvement_plan/run_phase1_group_m.py
```

Expected: エラーなし（または自動修正済み）

- [ ] **Step 3: 構文確認**

```powershell
python -c "import ast; ast.parse(open('backtest/backtest_improvement_plan/run_phase1_group_m.py').read()); print('OK')"
```
...
```bash
git add backtest/backtest_improvement_plan/run_phase1_group_m.py
git commit -m "feat: Group M バックテストスクリプト実装（スコア重み付け・銘柄集中検証）"
```
