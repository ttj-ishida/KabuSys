...
> **ステータス（2026-05-06時点）**: **全項目実装済み**  
> - 項目1（ダミーシグナル注入CLI）: **実装済み**（Issue #229 / `src/kabusys/tools/inject_dummy_signal.py`）  
> - 項目2（初期資金設定化）: **実装済み**（Issue #255 / `PAPER_TRADING_INITIAL_CASH` 環境変数）  
> - 項目3（MockBroker 状態復元）: **実装済み**（Issue #255 / `run_execution.py::_restore_paper_state`）  
> - 項目4（DBリセット機能）: **実装済み**（Issue #255 / `scripts/setup_db.py --paper-reset`）  
> - 項目5（kabu検証環境サンドボックス）: **実装済み**（Issue #255 / `KABU_USE_SANDBOX=true`）  
...
### [x] 1. 特定の銘柄のシグナルを注入するCLIツール（**実装済み** Issue #229）
...
### [x] 2. ペーパートレード用仮想資金の設定化（**実装済み** Issue #255）
...
### [x] 3. モックブローカーの状態復元機能（継続運用への対応）（**実装済み** Issue #255）
...
### [x] 4. テスト用DB環境のリセット機能（**実装済み** Issue #255）
...
### [x] 5. kabuステーション検証環境（ポート18081）を使ったペーパートレード対応（**実装済み** Issue #255）
