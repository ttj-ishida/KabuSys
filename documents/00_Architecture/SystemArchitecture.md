...
2. **優先度制御 (Priority)**: 執行系プロセスとkabuステーションプロセスにOSの「高優先度」を割り当て、AIタスク等の負荷スパイク時にも発注と損切りが必ず通るようにする。実装は `src/kabusys/utils/process_priority.py`（`set_process_priority()` / `set_cpu_affinity()`）が担い、各起動スクリプト（`run_execution.py`, `run_monitoring.py`）の先頭で呼び出す。Windows では `psutil.HIGH_PRIORITY_CLASS`、Linux では `os.nice(-10)` を使用してプラットフォーム差分を吸収する（Issue #43）。
